...
const other_pj = document.querySelector("#other-project");
...
document.body.addEventListener("change", function (e) {
  ...
  switch (target.id) {
    case "ntpc-yes":
      txtChp.removeAttribute("disabled");
      txtChp.style.backgroundColor = "#fff";
      break;
    case "ntpc-no":
      txtChp.setAttribute("disabled", true);
      txtChp.style.backgroundColor = "rgb(247,247,247)";
      break;
  }
});
...
project_opt.addEventListener("change", function () {
  if (project_opt.value === "other-pj") {
    other_pj.removeAttribute("disabled");
    other_pj.style.backgroundColor = "#fff";
    document.querySelector('input[name="ai"]:checked').checked = false;
  } else if (project_opt.value === "vanphong") {
    other_pj.setAttribute("disabled", true);
